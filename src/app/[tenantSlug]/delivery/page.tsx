...
export default function TenantDeliveryDashboardPage() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push(`/${tenantSlug}/auth/signin?callbackUrl=/${tenantSlug}/delivery`);
      return;
    }
    ...
    fetchOrders();
  }, [status, session, router, tenantSlug]);
  ...
  const fetchOrders = async () => {
    try {
      const res = await fetch(`/api/delivery/orders?tenantSlug=${tenantSlug}`);
      if (res.ok) {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    ...
    try {
      const res = await fetch(`/api/delivery/orders/${orderId}?tenantSlug=${tenantSlug}`, {
        method: 'PATCH',
