...
async function getUserOrdersWithReviews(userId: string, tenantId: string) {
...
export default async function TenantOrdersPage({
  ...
  if (!session) {
    redirect(`/${tenantSlug}/auth/signin?callbackUrl=/${tenantSlug}/orders`);
  }
  ...
  const { orders, reviewMap } = await getUserOrdersWithReviews(session.user.id, tenant.id);
        ...
        {orders.length > 0 ? (
          ...
          <Card>
            <CardContent className="py-16 text-center">
              ...
              <a href={`/${tenantSlug}/products`} className="text-green-600 hover:underline">
                Start shopping →
              </a>
            </CardContent>
          </Card>
