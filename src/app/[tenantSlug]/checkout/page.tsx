...
export default function CheckoutPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const cartRes = await fetch(`/api/cart?tenantSlug=${tenantSlug}`);
      if (cartRes.ok) {
      ...
      // Load loyalty info
      const loyaltyRes = await fetch(`/api/user/loyalty?tenantSlug=${tenantSlug}`);
      if (loyaltyRes.ok) {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      router.push(`/auth/signin?callbackUrl=/${tenantSlug}/checkout`);
      return;
    }
  ...
  }, [status, router, loadData, tenantSlug]);
  ...
  const handleCODOrder = async () => {
    ...
    try {
      ...
      await fetch(`/api/cart?tenantSlug=${tenantSlug}`, { method: 'DELETE' });
